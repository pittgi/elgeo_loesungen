...
\begin{exercise}{4.3}{3}
    ...
    $[p_{i-1}, p_i)$ paarweise disjunkt sind und $(p_{i+1} - pi, p_{i-1} - p_i)$ für alle $i$ linear unabhängig sind und als angeordnete Basen alle zu derselben Orientierung von $\vec{E}$
    gehören. So gilt in der Überlagerung $\tilde{\mathbb W}$ der Winkelgruppe
    \[
        \sum_{i=1}^{n}\angle (p_{i+1}-p_i, p_{i-1}-p_) = (n-2) 180^\circ
    \]
...
Nun wähle eine Orientierung auf dem Richtungsraum, sodass $F$ positiv ist.
Dann gilt:
\[
